...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!q) throw new Error('La consulta de Tavily está vacía.');
    ...
    await setJob(jobId, {
      status: 'running',
      provider: 'tavily',
      progress: 'Elix Search buscando fuentes en Tavily.',
    ...
    if (!response.ok && [400, 401, 403, 422].includes(response.status)) {
      await setJob(jobId, {
        status: 'running',
        provider: 'tavily',
        progress: 'Tavily reintentando autenticación compatible.',
      });
    ...
    if (!response.ok) {
      const err = new Error(`Tavily: ${upstreamMessage(data, `${response.status} ${response.statusText}`)}`);
    ...
    console.error('tavily-search-background', error);
    if (jobId) {
      try {
        await setJob(jobId, {
          status: 'error',
          provider: 'tavily',
          error: error?.message || 'Error interno en Tavily.',
          upstream_status: Number(error?.statusCode) || 500,
          finished_at: new Date().toISOString(),
        });
      } catch (storeError) {
        console.error('No se pudo guardar el error del job Tavily:', storeError);
      }
    }
  }
};
